...
class SafeTwitterDataCollector {
  ...
  private isMainListUserCell(element: Element): boolean {
    ...
    const isInSidebar =
      element.closest('[data-testid="sidebarColumn"]') !== null;
    if (isInSidebar) {
    ...
    const isInSuggestionBox =
      element.closest('[aria-label*="suggest"]') !== null ||
    ...
    const isInMainArea =
      element.closest('[data-testid="primaryColumn"]') !== null ||
  ...
  private extractUserFromElement(userElement: Element): TwitterUser | null {
    try {
      ...
      const imgElement = userElement.querySelector(
      ...
      const followsYouElement = userElement.querySelector(
        '[data-testid="userFollowIndicator"]'
      );
      const followsYou =
        followsYouElement?.textContent?.includes("Follows you") || false;
      ...
      const doesntFollowYou =
        followButtonText === "Follow" || followButtonText === "Follow back";

      // They follow you if they have the "Follows you" badge OR if they don't have a follow button
      const actuallyFollowsYou =
        followsYou || (!followButton && !doesntFollowYou);
      ...
      console.error(
        "[WhoUnfollowedMe] Error extracting user from element:",
        error
      );
      return null;
    }
  ...
  private highlightUnfollower(userElement: Element, username: string) {
    try {
      ...
      const userCell = userElement.closest(
  ...
  private collectVisibleUsers() {
    ...
    console.log(
      `[WhoUnfollowedMe] Found ${userElements.length} total user elements`
    );
    ...
    console.log(
      `[WhoUnfollowedMe] Filtered: ${
        userElements.length - mainListElements
      } suggestion/sidebar elements, ${mainListElements} main list elements`
    );
  ...
  private safeScroll() {
    ...
    console.log(
      `[WhoUnfollowedMe] Scrolled (${this.scrollCount}/${this.maxScrolls}), amount: ${scrollAmount}`
    );
    ...
    const contentLoadTimeout = setTimeout(() => {
      ...
      if (!this.isCollecting) {
        console.log(
          "[WhoUnfollowedMe] Collection stopped, skipping content collection"
        );
        return;
      }
      ...
      if (!this.isCollecting) {
        console.log(
          "[WhoUnfollowedMe] Collection stopped, not scheduling next scroll"
        );
        return;
      }
  ...
  public debug() {
    ...
    console.log(
      "- Unfollowers:",
      unfollowers.map((u) => `@${u.username} (${u.displayName})`)
    );
  ...
  public getAllUnfollowersWithLinks(): {
    username: string;
    displayName: string;
    profileUrl: string;
  }[] {
...
(window as any).getAllUnfollowersWithLinks = () =>
  collector.getAllUnfollowersWithLinks();
...
console.log(
  "- window.getUnfollowers() - Get list of users who don't follow you back"
);
console.log(
  "- window.getFollowersBack() - Get list of users who follow you back"
);
console.log(
  "- window.getAllUnfollowersWithLinks() - Get unfollowers with profile links"
);
console.log("- window.stopCollection() - Stop data collection manually");
console.log(
  "- window.isCollectionActive() - Check if collection is currently active"
);
console.log(
  "- window.forceStopAll() - Force stop all activities and clear data"
);
